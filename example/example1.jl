...
using LaTeXStrings
sim_plot = plot(time_plot/3600,O2_plot/O2_plot[1]*21,lw=4,label=[L"{\color{black}O_2}"]);
plot!(time_plot/3600,CO2_plot/O2_plot[1]*21,lw=4,label=[L"{\color{black}O_2}",L"{\color{black}CO_2}"])
plot!(xlabel = "Time (h)", ylabel = "Gas concentration (%)")
plot!(background_color=:transparent, foreground_color=:black)
plot!(title="Simulated output")
plot!(legendfontsize=16)
plot!(titlefontsize=28)
plot!(tickfontsize=16)
plot!(guidefontsize=22)
plot!(legend=:best)
png(sim_plot, "simulation")
...
t_plot = 0:1:3600*24
input = input_plotter(t_plot,matrix_inputs,tswitch)
t_plot_h = t_plot/3600
input_resize = copy(input)
input_resize[:,1] = input_resize[:,1]/(10.0/3600.0/1000.0)
input_resize[:,2] = input_resize[:,2]/O2_plot[1]*21
input_resize[:,3] = input_resize[:,3]/O2_plot[1]*21
out_plot1 = plot(t_plot_h,input_resize[:,2:3],lw=4,label=[L"{\color{black}O_2}",L"{\color{black}CO_2}"])
plot!(xlabel = "Time (h)", ylabel = "Gas concentration (%)")
plot!(background_color=:transparent, foreground_color=:black)
plot!(title="Gas inputs")
plot!(legendfontsize=16)
plot!(titlefontsize=28)
plot!(tickfontsize=16)
plot!(guidefontsize=22)
plot!(legend=:best)
png(out_plot1, "out_plot1")

out_plot2 = plot(t_plot_h,input_resize[:,1],lw=4,label="")
plot!(xlabel = "Time (h)", ylabel = "Flowrate l/h")
plot!(background_color=:transparent, foreground_color=:black)
plot!(title="Flowrate input")
plot!(legendfontsize=16)
plot!(titlefontsize=28)
plot!(tickfontsize=16)
plot!(guidefontsize=22)
plot!(legend=:best)
png(out_plot2, "out_plot2")
